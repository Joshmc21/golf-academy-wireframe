...
// === PART 6: ADVANCED ANALYTICS, ADMIN & ROLE DASHBOARDS ===

// 🧭 Role-Based Dashboard Routing
async function routeUserByRole(userId) {
  if (!userId) return;

  // Fetch user role from profiles or golfers table
  let role = "golfer";
  try {
    ...
    console.warn("Could not fetch user role, defaulting to golfer.", err);
  }

  console.log(`Routing user (${userId}) with role: ${role}`);
  ...
  if (role === "admin") {
    ...
    const golfer = await loadGolferFromDB(userId);
    if (golfer) renderGolferDashboard(golfer);
  }
}

// --- Draw Sparkline Utility (inline SVG, matches existing style) ---
function drawSparkline(values, width = 240, height = 60, color = "#2e86de") {
  if (!values || !values.length) return `<svg width="${width}" height="${height}"></svg>`;
  const max = Math.max(...values);
  const min = Math.min(...values);
...
function renderCorrelationsPage() {
  ...
  main.innerHTML = `
    <section>
      <h1>Correlations</h1>
      <p>How your physical metrics align with golf performance (Strokes Gained).</p>
      <table class="table">
        <tr><th>Metric</th><th>Correlation with SG Total</th></tr>
        <tr><td>Club Head Speed (CHS)</td><td style="color:${corrColor(chsCorr)}">${fmt(chsCorr)}</td></tr>
        <tr><td>Countermovement Jump (CMJ)</td><td style="color:${corrColor(cmjCorr)}">${fmt(cmjCorr)}</td></tr>
...
function renderTrendsPage() {
  ...
  main.innerHTML = `
    <section>
      <h1>Performance Trends</h1>
      <p>Track your metrics over time with sparklines below.</p>
...
function renderCoachDashboard() {
  ...
  main.innerHTML = `
    <section class="dashboard">
      ...
      <p>Manage your golfers and track their progress.</p>

      <div class="grid grid-2">
        <div class="card" onclick="renderCoachAthletes()">
          👟 <strong>View Golfers</strong>
        </div>
        <div class="card" onclick="renderCorrelationsPage()">
...
function renderCoachAthletes() {
  ...
  main.innerHTML = `
    <section>
      <h1>Your Golfers</h1>
      <p>Feature coming soon: pull assigned golfers from Supabase.</p>
      <button onclick="renderCoachDashboard()">⬅️ Back</button>
    </section>
  `;
}
...
function renderAdminPage() {
  ...
  main.innerHTML = `
    <section>
      <h1>Admin Dashboard</h1>
      <p>Manage data, users, and system status.</p>
      <div class="grid grid-3">
        <div class="card" onclick="renderAdminUsers()">👤 Users</div>
        <div class="card" onclick="renderAdminCycles()">📅 Cycles</div>
        <div class="card" onclick="renderAdminCompliance()">✅ Compliance</div>
...
function renderAdminUsers() {
  ...
  main.innerHTML = `
    <section>
      <h1>All Golfers</h1>
      <p>Future enhancement: query all golfers from Supabase 'golfers' table.</p>
      <button onclick="renderAdminPage()">⬅️ Back</button>
    </section>
  `;
}
...
function renderAdminCycles() {
  ...
  main.innerHTML = `
    <section>
      ...
      <p>Manage training cycles, deadlines, and notifications.</p>
      <button onclick="renderAdminPage()">⬅️ Back</button>
    </section>
  `;
}
...
function renderAdminCompliance() {
  ...
  main.innerHTML = `
    <section>
      <h1>Compliance Overview</h1>
      <p>See which golfers are overdue for updates.</p>
      <button onclick="renderAdminPage()">⬅️ Back</button>
    </section>
  `;
}
